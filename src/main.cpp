...
vision::signature BLUE_BALL =
    vision::signature(1, -4393, -2133, -3263, -1, 4567, 2283, 1.1, 0);
vision::signature RED_BALL =
    vision::signature(2, 6539, 10627, 8583, -2007, -1013, -1510, 2.5, 0);
...
// -------------------- ARM --------------------
const int ARM_UP = -0;    // degrees for arm up position
const int ARM_DOWN = 180; // degrees for arm down position
...
void toggleArm() {
  ...
  if (arm) {
    // Move arm up
    motor19.spinToPosition(ARM_UP, degrees, 200, velocityUnits::pct);
    arm = false;
  } else {
    // Move arm down
    motor19.spinToPosition(ARM_DOWN, degrees, 200, velocityUnits::pct);
    arm = true;
  }
...
void toggleWing() {
  ...
  if (wing) {
    if (!firstDeploy) {
      descore.spinToPosition(FIRST_DEPLOY_OUT, degrees, 100,
                             velocityUnits::pct);
      firstDeploy = true;
    } else {
...
void wingUp() { descore.spinToPosition(400, degrees, 100, velocityUnits::pct); }
...
/*void toggleShootingL() {
    ...
    if (shoot_l) {
        ...
        change_shooting.spin(reverse, 100, pct);// orignally the descore but wil
be used to set up the angle of descore wait(200, msec);
        change_shooting.stop(hold);
        shoot_l = true;
    }
...
void toggleShootingM() {
  ...
  if (shoot) {
    change_shooting.spinToPosition(100, degrees, 100, velocityUnits::pct);
    shoot = false;
  } else {
    change_shooting.spinToPosition(50, degrees, 100, velocityUnits::pct);
    shoot = true;
  }
...
void toggleShootingL() {
  ...
  if (shoot_l) {
    change_shooting.spinToPosition(60, degrees, 100, velocityUnits::pct);
...
void toggleMechClutch() { mech_clutch = !mech_clutch; }

// -------------------- MOTOR TEMP MONITOR --------------------
void updateMotorTemperatureColors() {
  double temps[] = {
      left_motor_f.temperature(celsius),  left_motor_b.temperature(celsius),
      left_motor_c.temperature(celsius),  right_motor_f.temperature(celsius),
      right_motor_b.temperature(celsius), right_motor_c.temperature(celsius)};
...
void drivercontrol() {
  ...
  while (true) {
    ...
    if (controller1.ButtonB.pressing()) {
      ...
      // middle
      change_shooting.spinToPosition(140, degrees, 100, velocityUnits::pct);
      ...
      change_shooting.spinToPosition(35, degrees, 100, velocityUnits::pct);
    }
      ...
      long_middle_b.stop();
      unclog.stop();
      change_shooting.spinToPosition(90, degrees, 100, velocityUnits::pct);
    }
...
void drive_cm(double distance_cm, double left_speed, double right_speed,
              bool intake_bool, double intake_wait) {
...
void driveSmooth(double distance_cm, int maxSpeedLeft, int maxSpeedRight,
                 double accelDist, double decelDist, bool intake_bool,
                 double intake_wait) {
...
// Smooth turn
void turnSmooth(double angle_deg, int maxSpeed, double accelAngle,
                double decelAngle) {
...
void turnSmooth1(double angle_deg, int maxSpeed, double accelAngle,
                 double decelAngle, bool intake_bool, double intake_wait) {
  int dir = (angle_deg >= 0) ? 1 : -1;
  angle_deg = fabs(angle_deg);
...
void autonomous() {
  ...
  //  driveSmooth(distance, leftSpeed, rightSpeed, accel, decel, isIntakeUsed?,
  //  for how long?);
